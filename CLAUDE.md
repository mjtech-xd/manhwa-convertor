...
**Left**
- *Phase 6 polish:* hit the p95 chapter <60 s *number* — the telemetry to measure it now exists (stage timing + p50/p95 in the Debug panel), but verifying/reaching the target needs live-API runs. Identified safe levers, not yet done: run `accuracy` concurrently with `assemble` (its `issues` aren't in the zip, so no golden-file risk); bounded-context narrate concurrency. Also worker-level streaming + backpressure (current streaming is IPC-boundary only). (Gemini prompt-response LRU + IndexedDB filtered-image cache + `MessageChannelMain` IPC streaming + stage-timing p50/p95 telemetry done.)
...
| 6 | Performance + polish           | 🚧 IN PROG  | **Done:** worker pool (ADR-001) for rasterise+filter; ESM/CJS fix so the desktop app boots (ADR-003); `build:all` lib chain + packaging fixed — `domain`→`@mc/domain`, hidden type errors, `asarUnpack` (ADR-004); `electron-builder --dir` produces a runnable app; `npm run typecheck` reworked to a real `tsc -b` (libs + renderer + specs + electron); initial bundle back under budget (jszip dynamic-imported → 404.8 kB); large image grids stay smooth via CSS `content-visibility: auto` (no CDK, zero bundle cost — Chromium-only is fine for Electron); Gemini prompt-response LRU (200/session) routes all five adapter calls through one cached `generate()` path — a cache hit skips the HTTP round-trip *and* the key pick, so resumed/retried runs spend no quota re-deriving identical output; IndexedDB filtered-image cache (`FilteredImageCacheService`) keyed on the ordered source-byte hashes + filter-settings hash — a hit on the `ImageFilterAdapter` skips the whole rasterise→sharp IPC round-trip, so re-running an identical chapter (resume/retry) reuses filtered output. Batch-level keying (not per-image) is deliberate: a page's `kept`/`reason` depends on the dedup lookback over preceding pages, so only an identical page sequence can safely reproduce the decisions. Best-effort store: no-ops when IndexedDB is absent; `pdf:rasterise` + `image:filter` now stream pages over a `MessageChannelMain` port (`electron/main/ipc/stream.ts` + preload `streamInvoke`) instead of one monolithic `ipcMain.handle` response — the renderer registers/renders each page as it lands. (No zero-copy: `MessagePortMain`'s transfer list only accepts ports, so byte payloads are still structured-clone copied — the win is incremental delivery + smaller messages, not avoiding the copy.) stage-timing telemetry (CLAUDE.md §12): `run-chapter.use-case` emits domain `StageEvent`s to `EVENT_BUS_PORT`; `StageTimingService` (eager-init via `provideInfrastructure`) pairs start/end per `(stage, runId)`, keeps a 200-sample window, derives p50/p95 behind `STAGE_TIMING_PORT`; the Debug panel renders the table + a chapter-p95-vs-60s-budget readout. **Left:** worker-level streaming + backpressure (current streaming is IPC-boundary only); hit the p95 <60 s number (needs live-API runs; safe levers: accuracy∥assemble, narrate concurrency). |
...
| `components/ImageGrid.tsx`                                         | `ui-kit/image-grid` (off-screen cells skipped via CSS `content-visibility: auto` — replaced the planned `@angular/cdk/scrolling`; zero bundle cost, keeps variable-height layout) |
...
- Long lists: `@for` with a stable `track` id. For large media grids prefer CSS `content-visibility: auto` (+ `contain-intrinsic-size`) over DOM virtualisation — it skips off-screen layout/paint with zero JS/bundle cost (Chromium-only, fine for Electron). Reach for `@angular/cdk/scrolling` only when you need true windowing of an unbounded list.
